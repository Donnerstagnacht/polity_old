...
DROP function if exists transactions.check_if_room_already_exists_and_follow(followerId uuid, followingId uuid);
DROP TYPE if exists check_if_room_already_exists_return_type;
CREATE TYPE check_if_room_already_exists_return_type AS (
    room_id uuid,
    count integer
);
create or replace function transactions.check_if_room_already_exists_and_follow(followerId uuid, followingId uuid)
returns boolean
language plpgsql
security definer
as
...
BEGIN
  ...
  if number_of_participants_in_room.count = 2 then
    ...
    perform transactions.update_accepted(number_of_participants_in_room.room_id, followerId);
    return true;
  else
    -- create room and set room invitation
    Select insert_room()
    into new_room_id;
    perform transactions.insert_participant(new_room_id, followerId, null, true);
    perform transactions.insert_participant(new_room_id, followingId, null, false);
    return false;
  END IF;
END;
$$;
